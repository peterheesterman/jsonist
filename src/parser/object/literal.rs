...
pub fn parse_literal(tokens: &Vec<Token>, position: usize) -> Result<JumpNode, FormatterError> {
    if let Some(value) = tokens.get(position) {
        match value {
            Token::StringLiteral(_, literal) => Ok((1, (Node::Literal { literal: literal.to_string() }))),
...
mod tests {
    ...
    fn parse_literal_normal() {
        ...
        match parse_literal(&vec![ literal ], 0) {
            Ok((_, result)) => assert_eq!(result, Node::Literal { literal: String::from("literal literal") }),
            Err(e) => panic!("{}", e),
        }
    }
